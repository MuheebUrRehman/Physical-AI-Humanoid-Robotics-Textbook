...
model: powerful-model-id           # Optional: model override
---
```
...
| `description` | Yes | ≤1024 characters; must include What + When | The agent uses this to decide when to trigger |
...
**Purpose**: The agent reads this to decide when to activate the skill. Include specific trigger phrases users would say.
...
```yaml
# For complex reasoning tasks (e.g., GPT-4o, Sonnet, Gemini Ultra)
model: powerful-model-id

# For simple, fast operations (e.g., GPT-4o-mini, Haiku, Gemini Flash)
model: fast-model-id
```
...
- ≤1024 characters
- The agent can match user intent
...
- Too vague for the agent to match
